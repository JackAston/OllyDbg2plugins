...
Function  ODBG2_Pluginquery(Const ollydbgversion: Int32; Const pluginname{SHORTNAME}, pluginversion{SHORTNAME}: PWideChar) : Int32; Cdecl;
Begin
  ...
  Addtolist(0, 1, '%s %s', PluginName, PluginVersion);
  Addtolist(0, 2, '  By '+Plugin_Auth);
  If (ollydbgversion < PLUGIN_VERSION) Then Begin
    Addtolist(0, 0, ' ');
    Addtolist(0, 1, 'Error : Expected Debugger Version %d, yours is %d ..  Upgrade now!', PLUGIN_VERSION, OllyDbgVersion);
    Addtolist(0, 0, ' ');
    Exit;
  End;
